...
%define stepfunction, 0 if y <0.5, 1 if y>=0.5;
StepFunc = [0,1,0.3];
BC = {BCbot BCtop BCleft BCright}; 
direction = 'X';
%BCType ={'PeriodicY','','Outlet','Inlet'};
...
IntByPart = 'no';
%BCType = {'flux','Strong','Strong','Strong'};
gamma = 1;
global CbI;
CbI = 4;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[TotalDOF,vertexData,edgeData,faceData,IBC,BCval,uHBCE,uHBCV,IENstruct,iper] = buildMeshStruct(xPoints,yPoints,xDomain,yDomain,x1,y1,x2,y2,OrderList); 
...
[u] = solveU(vertexData,IBC,BCval,IENstruct,iper,c1,edgeData,n1,n2,n3,n4,OrderList);
 c1Start = solvec1(OrderList,xPoints,yPoints,vertexData,faceData,IBC,uHBCE,uHBCV,nQ,IENstruct,u);
...
if itau == 3
    for iteration = 1:maxIter
        iteration
        [u] = solveU(vertexData,IBC,BCval,IENstruct,iper,c1,edgeData,n1,n2,n3,n4,OrderList);
        [c1new,L1new,L2new,M1new,M2new] = solvec1(OrderList,xPoints,yPoints,vertexData,faceData,IBC,uHBCE,uHBCV,nQ,IENstruct,u);
        c1new;
...
%maxY = 1.2;
surf(xu,yu,zz)
axis([minX maxX minY maxY -0.1 1.2])
view([-40 30]);
