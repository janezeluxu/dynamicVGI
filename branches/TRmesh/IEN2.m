    ...
    methods(Access = public)
        ...
        function [vertexIEN,edgeIEN,faceIEN] = Construct_IEN(ien)
            ...
            for i  = 1:Grid_size
                ...
                eID = meshData{i,3};
                if mod(i,2) == 1
                    edgeIEN{i,1} = [edgeData{eID(1),3}, edgeData{eID(1),4}];
                    edgeIEN{i,2} = [edgeData{eID(2),3},edgeData{eID(2),4}];
                    edgeIEN{i,3} = [edgeData{eID(3),3}, edgeData{eID(3),4}];
                %face uses edge in reverse order, will add usage
                %information later
                else
                    edgeIEN{i,1} = [edgeData{eID(1),3}, fliplr(edgeData{eID(1),4})];
                    edgeIEN{i,2} = [edgeData{eID(2),3}, fliplr(edgeData{eID(2),4})];
                    edgeIEN{i,3} = [edgeData{eID(3),3}, fliplr(edgeData{eID(3),4})]; 
                end
