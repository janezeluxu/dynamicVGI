classdef geo_Mesh < handle
    ...
    methods(Access = public)
        ...
        function [IBC,BCval] = BoundaryCondition2D(geoMesh,edgeData)
            %All boundary DOFs list
             global BC;
            M = geoMesh.XPoints;
            N = geoMesh.YPoints;
            ...
            for i = 1:length(vertexID1)
                if(i==1)
                    BCval1(count) = max(BC(1),BC(3));
                elseif (i==length(vertexID1))
                    BCval1(count) = max(BC(1),BC(4));
                else
                    BCval1(count) = BC(1);
                end
                count = count+1;
            ...
            for i = 1:length(vertexID2)
                if(i==1)
                    BCval2(count) = max(BC(2),BC(3));
                elseif (i==length(vertexID2))
                    BCval2(count) = max(BC(2),BC(4));
                else
                    BCval2(count) = BC(2);
                end
                count = count+1;
