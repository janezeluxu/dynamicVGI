...
nInt = max(pAll)+1;
[ShapeFuncTable,divSFtable] = ShapeTable(1,OrderList(1));

p = OrderList;
ShapeFuncTable = cell(length(p),1);
divSFtable = cell(length(p),1);
for j = 1:length(p)
    n = nIntergerPoints(p(j),0);
    [qPoint, weight] = GaussQuad(n, 1);
    qPoints(:,2) = qPoint;
    qPoints(:,3) = weight;
...
simplex = SimplexStiffMatrix(1,IENstruct,IENall,2,vertexData,ShapeFunc,divShapeFunc,c1Ele);
